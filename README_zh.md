...
│   └── table3_f12.csv        # 复现结果（CSV，随实验推进更新）
...
论文在 Gaussian / Student-t 噪声及 0–20% 响应离群点下给出了鲁棒回归基准。关键参考数值（测试 R²，EDERM(S,C)，引自论文）：

| 设置 | 0% 离群点 | 10% 离群点 | 20% 离群点 |
| ---- | --------- | --------- | --------- |
...
由 `experiments/run_tables.py` 产生的复现结果快照（10 次重复的平均测试 R²，采用与论文相同的噪声 / 离群点机制）：
...
- 数据生成遵循**原仓库语义**（离群点幅度：`f1` +N(20,1)、`f2` +t₂·10、`f3/f4` +t₂·1000）；论文正文未给出确切的污染幅度，因此 `f1` 的绝对数值不具备直接可比性。
- `f3/f4` 使用 `n = 250` 训练样本（论文为 1000），属于运行时间的折衷。
- 运行脚本后，所有设置（全部基线、两种噪声、全部离群点比例）的完整结果将写入 `experiments/*.csv`，日志实时输出到控制台。
